...
with open('position_and_mass_values.csv') as csvfile1:
...
with open('energy_values.csv') as csvfile2:
  read_energy = csv.reader(csvfile2, delimiter = ",")
...
def animate(i):
  ...
  for k in range((len(x[i]) - 2)//4):
    points1 = ax1.scatter(float(x[i][4*k]),float(x[i][(4*k)+1]),float(x[i][(4*k)+2]),color="blue", s = float(x[0][(4*k)+3]))
